...
## Chapter 5 - 总结
在正式开始前，决定好架构是重要的。在第一小组会议的时候用了大概３，４小时来讨论架构和模块间的对接。可以说这令我们在之后的正式作业时影响非常大。而在经过这一次的小组project，我们对书中和教授的内容有更深刻的了解。更清楚认识表是很重要的数据对象。表的定义好坏关付成败。表的内容在具体与繁琐之间的平衡。Ｂ＋树的调整也是在作业遇到最大的问题之一，数之不尽的ｂｕｇ，可以说Ｂ＋树是用了最多时间的部分也不为过。
