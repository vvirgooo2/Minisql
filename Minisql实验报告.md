...
张振宇：RecordManager + Interpreter +3/4 API+ 报告（对应详细设计模块，测试模块）

徐巧颖：BufferManager + 报告（对应详细设计模块，总体设计）

姜东甫：IndexManager + 报告 （对应详细设计模块，实验要求）

钟家伟：CatalogManager+ 1/4 API 报告（对应详细设计模块，实验总结）
...
首先说明Tuple的定义如下，Tuple存储一行sqlvalue内容。sqlvalue的定义能存储三种类型的值并标定。
...
CatalogManager中的LoadFromFile和WriteToFile分别是对DB文件的读和写；

  LoadFromFile把首先判定DB文件table.mata中是否存在对应表名metaFileNameh,再从文件table.catalog中把定义读取给table.

  WriteToFile把当前table的内容输出.把表名tablename写进DB文件table.mata,把定义table写进DB文件table.catalog.
...
```
  ...
  他们分别是:
  	...
  	创建表:检查各个value是否符合表定义并插⼊,调⽤IndexManager建立索引。
  	...
  	删除表:判断该表是否存在,若存在便删除这张表在内存中的定义，并删除相关的⽂件。
...
## Chapter 5 - 总结
在正式开始前，决定好架构是重要的。在第一小组会议的时候用了大概３，４小时来讨论架构和模块间的对接。可以说这令我们在之后的正式作业时影响非常大。而在经过这一次的小组project，我们对书中和教授的内容有更深刻的了解。更清楚认识表是很重要的数据对象。表的定义好坏关付成败,表的内容在具体与繁琐之间的平衡。Ｂ＋树的调整也是在作业遇到最大的问题之一，数之不尽的ｂｕｇ，可以说Ｂ＋树是用了最多时间的部分也不为过。本次实验采用C++语言进行编写，其中的数据结构全部应用了自定义的结构体及stl模板，保证了内存管理的安全性，也大大锻炼了我们对于C++语言的熟练度以及对于多文件编程的理解。本次实验使用了git作为版本控制系统，保证了上传同步的及时性，也能将记录完整地保存在云端，防止了代码数据的丢失和更新的不及时。本次实验耗费的总时间较长，小bug极多，排到最后也无法完全保证没有bug，这也证明了编写系统级软件的困难，但是也大大锻炼了我们的编程能力和对数据库运行过程的理解。
